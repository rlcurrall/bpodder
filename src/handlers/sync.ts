...
export function createSyncHandlers(ctx: AppContext): {
  syncDevices: RouteDefinition<"/api/2/sync-devices/:username">;
} {
  const getSyncStatus = (userId: number): SyncStatus => {
    ...
    syncDevices: {
      ...
      async POST(req) {
        try {
          ...
          const validation = validateDevicesExist(user.id, Array.from(allDeviceIds));
          if (!validation.valid) {
